...
/**
 * Hook que gerencia a conexão com a carteira MetaMask.
 *
 * Retorna:
 *   provider, signer, address    — objetos ethers.js para usar no contrato
 *   chainId, networkName         — rede atual
 *   isConnected, isSupportedNetwork
 *   isConnecting, error          — estado de UI
 *   connect(), disconnect()      — ações
 */
export function useWallet() {
  ...
  const networkName        = REDES_SUPORTADAS[chainId] ?? `Rede ${chainId}`;
  ...
  useEffect(() => {
    ...
    const onAccountsChanged = (accounts) => {
      ...
      else connect(); // reconecta com a nova conta
    };
